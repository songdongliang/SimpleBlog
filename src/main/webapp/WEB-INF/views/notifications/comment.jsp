				  ...
				  <div class="ui tab  active" data-tab="notread">
						<div class="ui relaxed list">
							<c:forEach items="${notis }" var="notification" begin="0" end="9">
							  <c:if test="${notification.notifyType eq dic.notify_type_comment }">
								  <div class="item">
								    ...
								    <div class="content">
								      ...
								      <c:if test="${notification.objectType eq dic.object_type_post }">
								      	<div class="description">评论了你的文章:<a href="<c:url value="/post/${notification.objectId }" />" ><b>${notification.objectTitle }</b></a> ${notification.ts }</div>
								      </c:if>
							  ...
							  <c:if test="${notification.notifyType eq dic.notify_type_comment_reply }">
								  <div class="item">
								    <img class="ui avatar image" src="<c:url value="${img_base_url }${notification.notifierAvatar }" />">
								    <div class="content">
								      <a class="header" href="<c:url value="/user/${notification.notifier }" /> ">${notification.notifierName }</a>
								      <div class="description">回复了你在:<a href="<c:url value="/post/${notification.objectId }" />" ><b>${notification.objectTitle }</b></a> ${notification.ts }</div>
								    </div>
								  </div>	
							  </c:if>	
							</c:forEach>
